...
main_loop:
	...
	show_num_1:
		...
		ori $t3, $t3, 56       # t3[2] <- 1, enabling first display. OBS: enable = t5(5 downto 2)
	...
	show_num_2:
		...
		lw $t3, 0($t3)         # t3 <- bcd_2
		sll $t3, $t3, 6        # t3 <- t3 << 6, 2 shifts for buttons and 4 for enable
		ori $t3, $t3, 52       # t3[3] <- 1, enabling second display. OBS: enable = t5(5 downto 2)
		sw $t3, 0($t2)         # Stores bcd code and enable in PortData

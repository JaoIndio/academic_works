...
.eqv KSS 0x10010c00 # Kernel Stack Start
.eqv PRIVATE_KEY 0x00003a41
.eqv N 0x0000ffc1

.eqv ENTER_ASCII_VALUE 13
...
i_not_zero_MEM:
  ...
  # Store new value on memory
  lw $t9, 0(%inst_value)  
  sll $t9, $t9, 8
  or %reg_to_save, %reg_to_save, $t9   
  sw %reg_to_save, 0(%inst_value)
  
  # If it's the last byte, save 4 bytes on memory
  # else, wait for next byte
  addiu $s2, $zero, 3
  bne $s1, $s2, wait_next_byte

  # check for NOP value
  beq %reg_to_save, $zero, NOP_detected

  # save whole instruction on memory and increment the address
  sw %reg_to_save, 0(%value_add)
  addiu %value_add, %value_add, 4
  j reset_index

  # If a NOP was detected, the application must change
  # Or just don't do anything
  NOP_detected:
      lw $t1, BidiPort_PortData_Addr          # t1 <= ConfigMode
      andi $t1, $t1, 0x00000003

      # If its data mode, do nothing
      bgtz $t1, reset_index

      # if its inst mode, change address to save APP 2
      la %value_add, INST_MEM_APP_2_ADDRESS
      j reset_index   
    
  # add +1 on index, save it, then end the function
...
rx_intr:
  ...
  # FIRST PREPARE AND LOAD DATA ON REGISTERS


  # t0 has the received byte
  lw $t0, RX_Addr

  # IMPORTANT: if t7 values 0, the enter character has been received
  addiu $t7, $zero, ENTER_ASCII_VALUE
  subu $t7, $t0, $t7
  
  # t1 and t2 are for byte index
  la $t1, rx_string_index
  lw $t2, 0($t1)

  # t3 and t4 are for the value to be saved on the string
  la $t3, rx_string_value
  lw $t4, 0($t3)


  # ACTUAL HANDLING STARTS BELOW

  # shifts t0 left by "index" bytes
  rx_shift_byte:

      beq $t2, $zero, rx_continue
      sll $t0, $t0, 8
      addiu $t2, $t2, -1
      j rx_shift_byte
  ...
  rx_continue:
      ...
      # get the index back and prepare the current value
      lw $t2, 0($t1)
      or $t4, $t4, $t0  

      # Save value on rx_string_value for later use
      # and increment the byte index
      sw $t4, 0($t3)
      addiu $t2, $t2, 1

      # if enter was received we must finish the string at once
      beq $t7, $zero, rx_save_word

      # check if a new full word has been received
      # if not, jump to the end of handler
      # 3 is the original
      #addiu $t5, $zero, 3
      addiu $t5, $zero, 4
      bne $t2, $t5, end_rx_intr

  rx_save_word:

      # get the destination address
      la $t5, rx_string_addr
      lw $t6, 0($t5)
      ...
      # reset the destination address if enter was detected
      bne $t7, $zero, reset_rx_string_index

      # reset the destination address
      reset_rx_string_addr:
        
          # make sure we end the string with zero
          sw $zero, ($t6)
...
end_IntToHexString:
  ...
  read:
    ...
    la $s0, rx_string           # address of source string in s0
    addiu $s1, $a0, 0           # addrees of destination in s1
    addiu $s2, $a1, 0           # number of bytes to read in s2
    addiu $s3, $zero, 0         # number of bytes already copied in s3
    
    read_loop:
    
        # load a word from the source address
        lw $s4, 0($s0)
        
        # s5 contains the number of bytes that remain to be read
        subu $s5, $s2, $s3
        
        # if there are still at least 4 bytes to be read, check for enter
        addiu $s6, $zero, 4
        subu $s6, $s5, $s6 
        bgez $s6, copy_word
        
        # if there are less than 4 bytes remaining
        # apply mask to s4
        
        # checks if 3 bytes remain
        addiu $s6, $zero, 3
        beq $s5, $s6, three_remaining
        
        # checks if 2 bytes remain
        addiu $s6, $zero, 2
        beq $s5, $s6, two_remaining   
        ...
        copy_word:
            ...
            # first we check for enter character
            addiu $s7, $zero, ENTER_ASCII_VALUE
            
            li $s6, 0x000000ff
            and $s6, $s4, $s6
            beq $s6, $s7, enter_on_first_byte
            
            li $s6, 0x0000ff00
            and $s6, $s4, $s6
            srl $s6, $s6, 8
            beq $s6, $s7, enter_on_second_byte
            
            li $s6, 0x00ff0000
            and $s6, $s4, $s6
            srl $s6, $s6, 16
            beq $s6, $s7, enter_on_third_byte
            
            li $s6, 0xff000000
            and $s6, $s4, $s6
            srl $s6, $s6, 24
            beq $s6, $s7, enter_on_fourth_byte
            
            j enter_not_detected
            
            enter_on_first_byte:
            
                # just store zero in the buffer
                addiu $s4, $zero, 0
                sw $s4, ($s1)
                j read_return_size
            
            enter_on_second_byte:
            
                # store the first byte followed by zeros in the buffer
                li $s6, 0x000000ff
                and $s4, $s4, $s6
                sw $s4, ($s1)
                addiu $s3, $s3, 1
            ...
            enter_on_third_byte:
                ...
                # store the first two bytes followed by zeros in the buffer
                li $s6, 0x0000ffff
                and $s4, $s4, $s6
                sw $s4, ($s1)
                addiu $s3, $s3, 2
            ...
            enter_on_fourth_byte:
            
                # store the first three bytes followed by zeros in the buffer
                li $s6, 0x00ffffff
                and $s4, $s4, $s6
                sw $s4, ($s1)
                addiu $s3, $s3, 3
            ...
            enter_not_detected:
                ...
                # checks if all bytes have been copied
                subu $s6, $s2, $s3 
                blez $s6, read_return_zero
      
                # if execution reaches here, neither size
                # bytes have been copied and neither has
                # the enter character been detected
         ...
         read_return_size:
            ...
            # number of bytes copied stored in v0
            addiu $v0, $s3, 0
            j reset_rx_string
         ...
         reset_rx_string:
            ...
            # s0 contains the size
            la $s0, rx_string_size
            lw $s0, 0($s0)
            
            # s1 contains rx_string address
            la $s1, rx_string

            # reset string value
            la $s2, rx_string_value
            sw $zero, 0($s2)

            # reset string index
            la $s2, rx_string_index
            sw $zero, 0($s2)
            
            # writes zeros all over rx_string
            loop_reset_string: 
            
                beq $s0, $zero, end_read
                sw $zero, 0($s1)
                addiu $s1, $s1, 4
                addiu $s0, $s0, -1
                j loop_reset_string
...
.data
  ...
  syscall_array:         .word  0 0 0 0

  # variables for memory programming
  i_byte_inst:           .word  0
  i_byte_data:           .word  0
  inst_add:              .word  INST_MEM_DYNAMIC_START_ADDRESS #
  data_add:              .word  DATA_MEM_DYNAMIC_START_ADDRESS # 0x100100a0
  inst_value:            .word  0
  data_value:            .word  0

  # variables for controling the received strings from rx
  rx_string:             .space 80
  rx_string_size:        .word  0x00000014
  rx_string_value:       .word  0x00000000 
  rx_string_addr:        .word  0x00000000
  rx_string_index:       .word  0x00000000 

  msg:                   .word  0
  printMsg:              .space 80
  string:                .space 4
  stringHex:             .word  9 8                            # 0x100100e8

  # DINAMIC ADD START # 0x100100ec

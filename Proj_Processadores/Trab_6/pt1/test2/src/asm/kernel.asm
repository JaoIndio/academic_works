...
i_not_zero_MEM:
  ...
  NOP_detected:
      ...
      li %value_add, INST_MEM_APP_2_ADDRESS
      j reset_index   
...
rx_intr:
  ...
  InstrucionMODE:
    la $t3, i_byte_inst
    la $t4, inst_add     # t4 = &inst_add
    lw $t5, 0($t4)       # t5 = *t4
    
    la $t6, inst_value   # t6 = *t5

    save_byte($t3, $t5, $t0, $t6)
    sw $t5, 0($t4) 
  ...
  DataMODE:
    la $t3, i_byte_data
    la $t4, data_add  # t4 = 0x100100a0    
    addiu $t5, $t5, 1
    lw $t5, 0($t4)        # t5 = 0x100100ec = DATA_MEM_DYNAMIC_START_ADDRESS  

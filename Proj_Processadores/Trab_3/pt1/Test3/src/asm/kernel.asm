...
.macro write_Birectional_Port()
	sll $s1, $s1, 1 
	sw $s1, 0x80000002  # PortData <- I_var 
.end_macro
...
peripheral_1:
	...
	# read_interruption_variable()
	la $s0, interruption_var
	lw $s1, 0($s0)           # s1 <= I_var
	# test_condition()
	addiu $s2, $zero, 255
	beq $s1, $s2, reestart	 # if (I_var == 255) I_var = 0
	
	addiu $s1, $s1, 1        # else I_var++
	sw $s1, 0($s0)
	j interrupt_end_function
	
reestart:
	add $s1, $zero, $zero
	sw $s1, 0($s0)
	
interrupt_end_function:

	write_Birectional_Port()
